...
import { addToJSONPointer, uriToJSONPointer, uriIsJSONPointer } from './utils.mjs';
...
class ResolveCrawler {
    constructor(runner, jsonPointer, _resolved) {
        ...
        this.resolvers = [];
        this.pointerGraph = new DepGraph({ circular: true });
        this.pointerStemGraph = new DepGraph({ circular: true });
        this.computeGraph = (target, parentPath = [], parentPointer = '#', pointerStack = []) => {
            ...
            if (ref !== undefined) {
                ...
                for (const key in target) {
                    ...
                    const currentPointer = addToJSONPointer(parentPointer, key);
        ...
        this._resolveRef = (opts) => {
            ...
            if (uriIsJSONPointer(ref)) {
                if (this._runner.dereferenceInline) {
                    const targetPointer = uriToJSONPointer(ref);
                    let targetPath;
                    try {
