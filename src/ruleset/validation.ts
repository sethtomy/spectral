...
import { IRuleDefinition } from './rule/types';
...
export function isValidRule(rule: FileRule): rule is IRuleDefinition {
  return typeof rule === 'object' && rule !== null && !Array.isArray(rule) && ('given' in rule || 'then' in rule);
}

export function assertValidRule(rule: FileRule): asserts rule is IRuleDefinition {
  if (!isValidRule(rule)) {
