...
import { isNPMSource, findFile } from './utils';
...
export class Ruleset {
  ...
  getFunctionByName(name: string): IFunction {
    const func = this.functions?.[name] ?? functions[name];
    if (func === void 0 || func === null) {
      throw new ReferenceError(`Function ${name} not found.`);
    }

    return 'compiled' in func ? func.compiled! : func;
  }
